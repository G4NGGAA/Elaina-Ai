document.addEventListener('DOMContentLoaded', () => {
    ...
    let chatHistory = [], currentChatId = null, attachedFiles = [], abortController = null;
    marked.setOptions({ breaks: true, gfm: true });

    const scrollToBottom = () => chatContainer.scrollTo({ top: chatContainer.scrollHeight, behavior: 'smooth' });
    const closeAllSidebars = () => { sidebarLeft.classList.remove('open'); sidebarRight.classList.remove('open'); overlay.classList.remove('active'); };
    const toggleSidebarLeft = () => { if (sidebarRight.classList.contains('open')) sidebarRight.classList.remove('open'); sidebarLeft.classList.toggle('open'); overlay.classList.toggle('active', sidebarLeft.classList.contains('open')); };
    const toggleSidebarRight = () => { if (sidebarLeft.classList.contains('open')) sidebarLeft.classList.remove('open'); sidebarRight.classList.toggle('open'); overlay.classList.toggle('active', sidebarRight.classList.contains('open')); };
    ...
    const decodeHTMLEntities = (text) => { const ta = document.createElement('textarea'); ta.innerHTML = text; return ta.value; };
        ...
        try {
            ...
            for (const support of supports) {
                ...
                const citationLinks = support.groundingChunkIndices
                    ...
                    .join('');
                if (citationLinks) processedText = processedText.slice(0, endIndex) + ' ' + citationLinks + processedText.slice(endIndex);
            }
            return processedText;
        } catch (e) { console.error("Error processing citations:", e); return text; }
    };
    ...
    const addMessageToUI = (msg, fullResponse = null) => {
        ...
        msg.parts.forEach(part => {
            if (part.file) contentHTML += part.file.mimeType.startsWith('image/') ? `<img src="${part.file.data}" class="chat-file-image">` : `<div class="chat-file-info"><span>${part.file.name}</span></div>`;
            if (part.text) {
                let textToParse = decodeHTMLEntities(part.text);
                if (msg.role === 'model' && fullResponse) textToParse = addCitations(textToParse, fullResponse.groundingMetadata);
        ...
        contentDiv.querySelectorAll('pre > code').forEach(codeElement => {
            ...
            header.querySelector('.download-btn').addEventListener('click', () => {
                const exts = { 'javascript': 'js', 'python': 'py', 'html': 'html', 'css': 'css', 'typescript': 'ts', 'json': 'json', 'markdown': 'md', 'text': 'txt'};
    ...
    const processAndSendMessage = async () => {
        ...
        const newUserMessage = { role: "user", parts: [], id: Date.now() };
        if(userMessage) newUserMessage.parts.push({ text: userMessage });
        userFiles.forEach(file => newUserMessage.parts.push({ file }));
        ...
        try {
            ...
            const res = await fetch('/chat', {
                ...
                body: JSON.stringify({ history: chatHistory.slice(0, -1), message: userMessage, files: userFiles, settings }),
                signal
            });
            ...
            const newAiMessage = { role: "model", parts: [{ text: fullResponse.text }], id: Date.now() };
            ...
            if (error.name !== 'AbortError') {
                const errorMsg = { role: "model", parts: [{ text: `Aww, maaf, Elaina error nih: ${error.message}` }], id: Date.now() };
    ...
    chatForm.addEventListener('submit', (e) => { e.preventDefault(); if (!document.body.classList.contains('is-streaming')) processAndSendMessage(); });
    ...
    fileInput.addEventListener('change', (e) => {
        ...
        if (attachedFiles.length + files.length > 5) { alert("Maaf, kamu cuma bisa upload maksimal 5 file yaa."); return; }
        files.forEach(file => {
            const reader = new FileReader();
            reader.onloadend = () => { attachedFiles.push({ data: reader.result, mimeType: file.type, name: file.name }); renderFilePreviews(); };
    ...
    const renderFilePreviews = () => {
        ...
        document.querySelectorAll('.remove-file-btn').forEach(btn => btn.addEventListener('click', (e) => { attachedFiles.splice(parseInt(e.target.dataset.index, 10), 1); renderFilePreviews(); }));
    };
    ...
    historyList.addEventListener('click', (e) => { const historyItem = e.target.closest('.history-item'); if (historyItem) loadChat(historyItem.dataset.chatId); });

    const loadLatestChat = () => {
        const latestChatId = localStorage.getItem('elaina-active-chat-id');
        const chats = getChats();
        if (latestChatId && chats[latestChatId]) { loadChat(latestChatId); } else { startNewChat(); }
    };

    window.addEventListener('beforeunload', () => { if (currentChatId) localStorage.setItem('elaina-active-chat-id', currentChatId); });

    const saveSettings = () => { const settings = { model: modelSelect.value, systemInstruction: systemInstruction.value, grounding: groundingToggle.checked, darkTheme: darkThemeToggle.checked }; localStorage.setItem('elaina-chan-settings', JSON.stringify(settings)); };
